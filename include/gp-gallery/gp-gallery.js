                ...
                pic.hover(function() {
                    ...
                    if (item.original_height > item.height && item.original_width > item.width) {
                        pic.is_hover = setTimeout(function() {
                            ...
                            if (pic.is_hover) {
                                // alert((item.original_width - item.width)/2);
                                var obj_offset = pic.offset();
                                var obj_left = (item.original_width - item.width)/2;

                                //Если картинка в полном размере заезжает за левый край экрана
                                if ((obj_offset.left - obj_left) < 0) {
                                    obj_left = obj_offset.left;
                                }

                                //Если заезжает за правый - доработать!
                                if ((obj_offset.left - obj_left + item.original_width) > window.innerWidth) {
                                    obj_left = window.innerWidth - (obj_offset.left - obj_left + item.original_width);
                                    obj_left = (item.original_width - item.width)/2 - obj_left;
                                }

                                pic.animate({
                                    marginTop: '-' + (item.original_height - item.height)/2 + 'px',
                                    marginLeft: '-' + obj_left + 'px',
